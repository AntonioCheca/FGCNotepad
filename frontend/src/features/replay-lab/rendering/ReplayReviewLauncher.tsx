            ...
            <SectionCard title="Source File" tone="raised" variant="input">
                <AppStack spacing={{xs: 0.8, md: 1.1}}>
                    ...
                    <AppStack direction={{xs: "column", sm: "row"}} spacing={1} alignItems={{xs: "stretch", sm: "center"}}>
                        ...
                        <AppTypography color="text.secondary" sx={{overflowWrap: "anywhere"}}>
            ...
            <SectionCard title="Resume Draft" tone="sunken" variant="finalize">
                <AppStack spacing={1}>
                    ...
                    {visibleSessions.map((session) => {
                        ...
                        return (
                            <AppBox key={session.id} sx={(theme) => ({display: "grid", gridTemplateColumns: {xs: "1fr", md: "1fr auto"}, gap: {xs: 0.75, md: 1}, alignItems: "center", p: {xs: 0.85, md: 1}, border: "1px solid", borderColor: theme.fgc.border.default, borderRadius: 1.25, backgroundColor: theme.fgc.surface.base, minWidth: 0})}>
                                <AppBox sx={{display: "grid", gap: 0.15, minWidth: 0}}>
                                    <AppTypography variant="subtitle2" sx={{overflowWrap: "anywhere"}}>{session.title}</AppTypography>
                                    <AppTypography variant="body2" color="text.secondary" sx={{overflowWrap: "anywhere"}}>{label}{video ? ` - ${video.originalFilename}` : ""} - Updated {formatUtcDateTime(session.updatedAt, "No expiry")}</AppTypography>
                                </AppBox>
