...
export function ReplayYouTubePlayer({videoId, fps, title, seekCommand, onPlaybackPositionChange, timelineAddon, controlsAddon}: ReplayYouTubePlayerProps) {
    ...
    React.useEffect(() => {
        ...
        const intervalId = window.setInterval(() => {
            ...
            const nextPosition = {timeMs, frame: Math.round((timeMs / 1000) * fps), durationMs: duration};
    ...
    React.useEffect(() => {
        if (!isReady || !seekCommand || lastSeekRef.current === seekCommand.id) {
            return;
        }
        lastSeekRef.current = seekCommand.id;
        playerRef.current?.seekTo(seekCommand.timeMs / 1000, true);
    }, [isReady, seekCommand]);
